...
---
**Ziel:** Modellierung und Optimierung der Sanierungszyklen von Belägen und Leitungen über 100 Jahre zur Identifikation von Synergien und Konflikten.  
...
- Einlesen der in den MCA Bäume und Beläge generierten outputfiles *mca_top3_baeume_pro_szenario.csv* und *mca_top3_bealege_pro_szenario.csv* zur Anzeige der "Gewinner" Bäume und Beläge pro Szenario. 
- Einlesen der tidy Daten für Bäume und Beläge für den Abruf der Lebensdauer
- Einlesen der tidy Daten der Leitungen für die Berechnung der Ankerjahre und die Bündelungen

Input: 

- [mca_top3_belaege_pro_szenario.csv](02_export/tables/mca_top3_belaege_pro_szenario.csv)
- [mca_top3_baeume_pro_szenario.csv](02_export/tables/mca_top3_baeume_pro_szenario.csv)
- [Belaege_tidy.csv](01_data_input/Belaege_tidy.csv)
- [Baeume_tidy.csv](01_data_input/Baeume_tidy.csv)
- [Leitungen_IST.csv](01_data_input/Leitungen_IST.csv)

```{r}
# --- A. Input 1: Die "Sieger" (Ergebnis aus MCA Bäume und Beläge) ---
...
# Hier müssen Spalten wie 'Leitungstyp' und 'Techn_Nutzungsdauer_a' stehen
colnames(df_leitungen_roh)

```

## 2. Daten Aufbereiten
Ziel: Die Daten aller Komponenten (Bäume, Beläge, Leitungen) für die Verrechnung vorbereiten.
...
df_leitungen <- df_leitungen_roh %>%
  ...
  # "Grabenlos" ist kein Konflikt für Bäume/Beläge und wird ignoriert.
...
Ziel: Eine *Master-Tabelle* erstellen, die Jede Komponente (Bäume, Beläge, Leitungen) für Jedes Szenario enthält und die Information enhält wer der *Sieger-Belag* des Szenarios ist. 
...
# Die Input-CSVs sind sortiert (Platz 1 oben)
...
Ziel: Alle Sanierungsevents pro Szenario für alle Komponenten (Bäume, Beläge, Leitungen) für 100 Jahre berechnen.
...
Ziel: Vergleich der Sanierungs-Szenarien zur Beantwortung von H1 & H2. Der Vergleich erfolgt jeweils für die technische Nutzungsdauer und die materielle Lebensdauer. 
...
Ziel: Anwendung des zweistufigen Optimierungs-Algorithmus zur Bündelung der Sanierungszeitpunkte. Synchronisation flexibler Komponenten mit Tiefbau-Ankern (vertikal) und Abstimmung der Beläge untereinander (horizontal) zur Minimierung der Strassenaufbrüche.
