---
title: "MCA Beläge"
author: "Sarah Pfeiffer"
date: today
format: 
    html:
        toc: true
        code-fold: false

---

## MCA für Szenario 0 bis 3

**Ziel:** Wahl der am besten geeigneten Beläge pro Szenario mit Hilfe einer MCA Berechnung.

- **Szenario 0:** Baseline (IST-Analyse): Simuliert die aktuellen Projektanforderungen. Fokus auf Nutzungsqualität, Wirtschaftlichkeit und Langlebigkeit.

- **Szenario 1:** Forschungsfokus: Balanciert Lebenszyklus-Fokus (H1/H2) und Vermeidungsprinzip (H3).

- **Szenario 2:** Radikale Vermeidung: Testet Hypothese 3 (H3) im Extrem (Fokus Umwelt & Kreislauf).

- **Szenario 3:** Radikaler Lebenszyklus: Testet Hypothesen 1 & 2 (H1/H2) im Extrem (Fokus Langlebigkeit).

## Libraries laden

```{r}

library(tidyverse)
library(readr)
library(ggplot2)
library(forcats)
library(ggtext)
library(tidytext)
library(patchwork)

```

## 1. Daten Importieren

Einlesen der Rohdaten für Beläge und Gewichtung für Szenario 0 bis 3

Input: 

- [Belaege_tidy.csv](01_data_input/Belaege_tidy.csv)
- [Belaege_Gewichtung_Szenario0.csv](01_data_input/Belaege_Gewichtung_Szenario0.csv)
- [Belaege_Gewichtung_Szenario1.csv](01_data_input/Belaege_Gewichtung_Szenario1.csv)
- [Belaege_Gewichtung_Szenario2.csv](01_data_input/Belaege_Gewichtung_Szenario2.csv)
- [Belaege_Gewichtung_Szenario3.csv](01_data_input/Belaege_Gewichtung_Szenario3.csv)

```{r}

## 1. Daten Importieren

# Liest die Rohdaten für die Beläge ein
df_belaege_raw <- read_delim(
  "01_data_input/Belaege_tidy.csv",
  delim = ";"
)

# Liest die Gewichtungen pro Szenarien ein
weights_s0_raw <- read_delim(
  "01_data_input/Belaege_Gewichtung_Szenario0.csv",
  delim = ";"
)
weights_s1_raw <- read_delim(
  "01_data_input/Belaege_Gewichtung_Szenario1.csv",
  delim = ";"
)
weights_s2_raw <- read_delim(
  "01_data_input/Belaege_Gewichtung_Szenario2.csv",
  delim = ";"
)
weights_s3_raw <- read_delim(
  "01_data_input/Belaege_Gewichtung_Szenario3.csv",
  delim = ";"
)
```

```{r}
#| include: false
# --- PRÜFUNG BLOCK 1 ---

print("--- 1. Check: Spaltennamen Beläge (Sind alle Spaltennamen da?) ---")
colnames(df_belaege_raw)

print("--- 2. Check: Gewichtung S0 (Wurden Spalten getrennt?) ---")
head(weights_s0_raw)

print("--- 3. Check: Gewichtung S3 (Zur Sicherheit) ---")
head(weights_s3_raw)

```

## 2. Daten bereinigen und harmonisieren

Ziel: Daten für den Join und anschliessend für die MCA Berechnung vorbereiten.

```{r}
# A. Bereinigung der Rohdaten
df_belaege <- df_belaege_raw %>%
  select(belagskategorie, option, everything()) 

# B. Bereinigung der Gewichtungen (alle Szenarien)
# Wir fügen den Roh-Dateien die Szenario-Namen hinzu
df_weights_s0 <- weights_s0_raw %>%
  mutate(szenario = "Szenario 0: Baseline (IST)")
df_weights_s1 <- weights_s1_raw %>%
  mutate(szenario = "Szenario 1: Forschungsfokus")
df_weights_s2 <- weights_s2_raw %>%
  mutate(szenario = "Szenario 2: Radikale Vermeidung")
df_weights_s3 <- weights_s3_raw %>%
  mutate(szenario = "Szenario 3: Radikaler Lebenszyklus")

# Jetzt binden wir sie zu einer finalen Gewichtungs-Tabelle zusammen
df_weights <- bind_rows(df_weights_s0, df_weights_s1, df_weights_s2, df_weights_s3) %>%
  # Berechne das Gesamtgewicht (gewicht) aus den Rohspalten,
  mutate(
    gewicht = group_weight * within_group_weight
  ) %>%
   # umbenennen für den Join
  rename(
    bewertungskategorie = criterion_id
  ) %>%
  # Auswählen der Spalten, die der Rest des Skripts braucht
  select(szenario, bewertungskategorie, gewicht, group_id)

```


```{r}
#| include: false
# --- PRÜFUNG BLOCK 2 ---

print(df_weights, n = 44)

print("--- Check: Stichprobe der Gewichte ---")
# Zeig mir mal S0 und S3 im Vergleich für Kosten
df_weights %>% 
  filter(bewertungskategorie == "initialkosten_100a") %>%
  select(szenario, gewicht)

```

## 3. Datentransformation

Ziel: Transformation der Tabelle *df_belaege* von Wide-to-Long und anschliessende Verknüpfung mit der Tabelle *df_weights* über join.

```{r}
# 3.1 Wide-to-Long
df_belaege_long <- df_belaege %>%
  pivot_longer(
    cols = where(is.numeric), 
    names_to = "bewertungskategorie", 
    values_to = "wert"
  )

# 3.2 JOIN mit Gewichtung
# Wir nutzen left_join. 
# Alles was in der Gewichtungstabelle fehlt (wie initialkosten), bekommt hier gewicht = NA.
df_belaege_joined <- df_belaege_long %>%
  left_join(df_weights, by = "bewertungskategorie", relationship = "many-to-many")

```


```{r}
#| include: false
# --- PRÜFUNG BLOCK 3 ---

print(df_belaege_joined, n = 44)

print("--- Check: Hat 'initialkosten' KEIN Gewicht (NA)? ---")
df_belaege_joined %>%
  filter(bewertungskategorie == "initialkosten") %>%
  select(szenario, bewertungskategorie, gewicht) %>%
  distinct() %>%
  head(2) # Sollte NA zeigen (oder leer sein, falls S0)

print("--- Check: Hat 'initialkosten_100a' ein Gewicht? ---")
df_belaege_joined %>%
  filter(bewertungskategorie == "initialkosten_100a") %>%
  filter(szenario == "Szenario 0: Baseline (IST)") %>%
  select(szenario, bewertungskategorie, gewicht) %>%
  distinct() %>%
  head(2) # Sollte 0.1 zeigen

```

## 4. Explorative Datenanalyse

### 4.1 Rohwerte Verteilung

Ziel: Die Verteilung der Rohwerte pro Kriterium ansehen.
Dies zeigt, warum eine Normierung (Block 5) zwingend notwendig ist,
da die Skalen Jahre, CHF, kg völlig unterschiedlich sind und somit nicht miteinander verglichen werden können.

```{r}
# -------------------------------------------------------
# TEIL 1: Boxplot der Rohwerte (Überblick)  (Scientific Style)
# -------------------------------------------------------
# Hinweis: Da die Einheiten gemischt sind, nutzen wir facet_wrap mit freien Skalen.
plot_rohwerte <- ggplot(df_belaege_long, aes(y = wert)) +
  geom_boxplot(fill = "grey95", width = 0.5, outlier.size = 1) +
  
  # Facetten für jedes Kriterium
  facet_wrap(~bewertungskategorie, scales = "free", ncol = 3) +
  
  labs(
    title = NULL, 
    subtitle = NULL,
    x = NULL,
    y = "Rohwert (unterschiedliche Einheiten)"
  ) +
  
  theme_classic(base_size = 11) +
  
  theme(
    # Nur horizontale Gitterlinien (y) sind wichtig zum Ablesen der Werte
    panel.grid.major.y = element_line(color = "grey90", linewidth = 0.2),
    panel.grid.major.x = element_blank(),
    
    # Facetten-Label
    strip.background = element_blank(),
    strip.text = element_text(face = "plain", size = 9),
    
    # Y-Achse (Werte) MUSS sichtbar sein:
    axis.text.y = element_text(size = 8, color = "black"), 
    
    # X-Achse (unten) kann leer sein:
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank()
  )

# print(plot_rohwerte)

```


```{r}
#| include: false
# Speicher des Plots
ggsave("02_export/plots/check_01_rohwerte_verteilung_belaege.png", 
       plot = plot_rohwerte, width = 10, height = 8, dpi = 300)

```

![Rohdaten-Indikatoren der 10 Belagsoptionen vor Normierung](02_export/plots/check_01_rohwerte_verteilung_belaege.png)

### 4.2 Kosten pro Belag auf 100 Jahre

Ziel: Die Initialkosten und Unterhaltskosten pro Belag auf 100 Jahre hochgerechnet ansehen. Erst die Hochrechnung auf 100 Jahre (mehrere Sanierungszyklen) zeigt die wahren Kosten. 


```{r}
#| label: fig-lebenszyklus-final-clean
#| fig.width: 10
#| fig.height: 12
#| out.width: "100%"
#| warning: false


# -------------------------------------------------------
# TEIL 2: KOSTEN (Gestapelt: Erneuerungszyklen + Unterhalt)
# -------------------------------------------------------

# 1. Daten vorbereiten
df_kosten_calc <- df_belaege %>%
  select(belagskategorie, option, initialkosten, initialkosten_100a, unterhalt_100a) %>%
  mutate(
    n_zyklen = round(initialkosten_100a / initialkosten),
    total_kosten = initialkosten_100a + unterhalt_100a
  )

# 2. Datensatz "aufblasen"
df_invest_expanded <- df_kosten_calc %>%
  uncount(n_zyklen) %>%
  mutate(
    kostenart = "Investitionskosten (pro Erneuerungszyklus)",
    wert = initialkosten,
    label_text = as.character(round(initialkosten, 0))
  )

df_unterhalt <- df_kosten_calc %>%
  mutate(
    kostenart = "Unterhaltskosten (kumuliert 100a)",
    wert = unterhalt_100a,
    label_text = as.character(round(unterhalt_100a, 0)) 
  )

df_plot_kosten_final <- bind_rows(df_invest_expanded, df_unterhalt) %>%
  mutate(option_sortiert = reorder(option, initialkosten_100a + unterhalt_100a)) %>%
  mutate(kosten_label = fct_relevel(kostenart, 
                                    "Unterhaltskosten (kumuliert 100a)", 
                                    "Investitionskosten (pro Erneuerungszyklus)"))

# 3. Plot Kosten erstellen
plot_kosten_stacked <- ggplot(df_plot_kosten_final, 
       aes(x = option_sortiert, y = wert, fill = kosten_label)) +
  
  # Balken mit weissen Trennlinien
  geom_col(width = 0.7, color = "white", linewidth = 0.3) + 
  
  # Beschriftung: Mittig im Block
  geom_text(aes(label = label_text), 
            position = position_stack(vjust = 0.5), 
            size = 2.5, 
            color = "white") +
  
  scale_fill_manual(values = c(
    "Investitionskosten (pro Erneuerungszyklus)" = "#084594", 
    "Unterhaltskosten (kumuliert 100a)" = "#9ecae1"
  )) +
  
  scale_y_continuous(breaks = seq(0, 3000, 200)) + 
  scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 20)) +
  
  labs(
    title = NULL,
    x = NULL,
    y = "Kosten [CHF/m²]",
    fill = NULL 
  ) +
  
  theme_classic(base_size = 11) +
  
  theme(
    legend.position = "bottom",
    panel.grid.major.y = element_line(color = "grey90", linewidth = 0.2),
    panel.grid.major.x = element_blank(),
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10, color = "black"),
    axis.text.y = element_text(size = 10, color = "black"),
    plot.margin = margin(t = 5, r = 5, b = 10, l = 5, unit = "mm")
  ) +
  guides(fill = guide_legend(reverse = TRUE))

# print(plot_kosten_stacked)
```


```{r}
#| include: false
# Speichern Kosten
ggsave("02_export/plots/Abb_Lebenszyklus_Kosten_Stacked_Zyklen.png", 
       plot = plot_kosten_stacked, width = 8, height = 8, dpi = 300)
```

![Investitions- und Unterhaltskosten pro Belag auf 100 Jahre](02_export/plots/Abb_Lebenszyklus_Kosten_Stacked_Zyklen.png)

### 4.3 Ökologische Bilanz po Belag auf 100 Jahre

Ziel: Die ökologischen Auswirkungen (CO2-Emissionen und Graue Energie) pro Belag hochgerechnet auf 100 Jahre ansehen. Erst die Hochrechnung auf 100 Jahre zeigt die wahren ökologischen Einflüsse. 


```{r}
# -------------------------------------------------------
# TEIL 3: UMWELT 
# -------------------------------------------------------

# --- Plot A: CO2-Emissionen ---
df_co2 <- df_belaege %>%
  select(belagskategorie, option, wert = co2_100a) %>%
  mutate(kategorie = fct_relevel(belagskategorie, "Asphalt", "Pflaster", "Sickerbeläge")) %>%
  mutate(option_sortiert = reorder(option, wert))

p_co2 <- ggplot(df_co2, aes(x = option_sortiert, y = wert, fill = kategorie)) + 
  geom_col(width = 0.7) + 
  
  scale_fill_manual(values = c("Asphalt" = "#084594", "Pflaster" = "#4292c6", "Sickerbeläge" = "#9ecae1")) +
  
  scale_y_continuous(breaks = seq(0, 1000, 50)) +
  
  scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 20)) +
  
  labs(title = "CO\u2082-Emissionen (100a)", y = "kg CO\u2082-eq/m\u00B2", x = NULL) +
  
  theme_classic(base_size = 11) +
  theme(
    legend.position = "none", 
    plot.title = element_text(face = "plain", size = 11, hjust = 0),
    axis.title.y = element_text(face = "plain", size = 10, color = "black"),
    panel.grid.major.y = element_line(color = "grey90", linewidth = 0.2), 
    panel.grid.major.x = element_blank(),
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10, color = "black"),
    # Margin unten erhöhen, damit Platz für den Text ist
    plot.margin = margin(t = 5, r = 5, b = 10, l = 5, unit = "mm")
  )


# --- Plot B: Graue Energie ---
df_grau <- df_belaege %>%
  select(belagskategorie, option, wert = graueEnergie_100a) %>%
  mutate(kategorie = fct_relevel(belagskategorie, "Asphalt", "Pflaster", "Sickerbeläge")) %>%
  mutate(option_sortiert = reorder(option, wert))

p_grau <- ggplot(df_grau, aes(x = option_sortiert, y = wert, fill = kategorie)) + 
  geom_col(width = 0.7) + 
  
  scale_fill_manual(values = c("Asphalt" = "#084594", "Pflaster" = "#4292c6", "Sickerbeläge" = "#9ecae1")) +
  
  scale_y_continuous(breaks = seq(0, 4000, 200)) +
  
  scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 20)) +
  
  labs(title = "Graue Energie (100a)", y = "kWh/m\u00B2", x = NULL, fill = "Belagskategorie") +
  
  theme_classic(base_size = 11) +
  theme(
    legend.position = "bottom",
    plot.title = element_text(face = "plain", size = 11, hjust = 0),
    axis.title.y = element_text(face = "plain", size = 10, color = "black"),
    panel.grid.major.y = element_line(color = "grey90", linewidth = 0.2), 
    panel.grid.major.x = element_blank(),
    
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10, color = "black"),
    
    plot.margin = margin(t = 10, r = 5, b = 5, l = 5, unit = "mm")
  )

# --- Zusammenfügen ---
# Patchwork regelt das Layout automatisch. 
plot_umwelt_combined <- p_co2 / p_grau

# print(plot_umwelt_combined)
```


```{r}
#| include: false
ggsave("02_export/plots/Abb_Belaege_Umwelt_clean.png", 
       plot = plot_umwelt_combined, width = 10, height = 14, dpi = 300)
```

![Ökologische Lebenszyklusbilanz der Belagsoptionen](02_export/plots/Abb_Lebenszyklus_Umwelt_clean.png)

## 5. MCA - Normalisierung

Ziel: 

- Normalisierung: Die Wert-Spalte die aktuell Jahre, CHF, etc. als Einheiten hat, muss auf eine einheitliche Skala (0-1) gebracht werden.
- Anschliessend wird eine Min-Max-Skalierung gemacht.Wie gut ist ein Belag im entsprechenden Kriterium im Vergleich zu allen anderen Belägen? 


```{r}
# 5.1 Definieren: Wo ist "weniger" besser?
kriterien_lower_is_better <- c(
  "initialkosten", 
  "initialkosten_100a", 
  "unterhalt_100a", 
  "co2_100a", 
  "graueEnergie_100a",
  "oberflaechentemperatur" 
)

# 5.2 Min-Max-Skalierung
# Wir normieren ALLES (auch ungewichtete Infos), damit wir sie plotten können.
df_belaege_normalized <- df_belaege_joined %>%
  filter(!is.na(wert)) %>% # Nur Zeilen ohne Werte löschen
  group_by(bewertungskategorie) %>%
  mutate(
    min_wert = min(wert, na.rm = TRUE),
    max_wert = max(wert, na.rm = TRUE),
    # Berechnung des Scores
    score_normiert = case_when(
      # Sonderfall: Wenn Min und Max gleich sind (alle gleich gut) -> Score 1
      (max_wert - min_wert) == 0 ~ 1,
      # Normale Skalierung (higher is better)
      !(bewertungskategorie %in% kriterien_lower_is_better) ~ (wert - min_wert) / (max_wert - min_wert),
      # Invertierte Skalierung (lower is better)
      bewertungskategorie %in% kriterien_lower_is_better ~ 1 - ((wert - min_wert) / (max_wert - min_wert))
    )
  ) %>%
ungroup()

```


```{r}
#| include: false
# --- PRÜFUNG BLOCK 5 ---

print("--- Check: Sind alle Kriterien da (auch die ohne Gewicht) und ist der Score Normiert zwischen 0 und 1? ---")
df_min_max_check <- df_belaege_normalized %>%
  select(bewertungskategorie, min_wert, max_wert, score_normiert) %>%
  distinct()

print(df_min_max_check)

print("--- Check: Haben 'initialkosten' einen Score bekommen? ---")
# Wir testen eine Zeile stichprobenartig
df_belaege_normalized %>%
  filter(bewertungskategorie == "initialkosten") %>%
  select(option, wert, score_normiert) %>%
  head(3)

# Prüfen, ob der Join geklappt hat (Gewicht da?):
print("--- Check Gewicht für Kosten ---")
df_belaege_normalized %>%
  filter(bewertungskategorie %in% c("initialkosten", "initialkosten_100a")) %>%
  select(szenario, bewertungskategorie, gewicht) %>%
  distinct() %>%
  print()
```

## 6. Explorative Datenanalyse

Ziel: Überprüfen, ob die Min-Max-Skalierung (Block 5.2) funktioniert hat.

```{r}
# Boxplot der normierten Scores (Scientific Style)
# Ziel: Überprüfen, ob die Min-Max-Skalierung funktioniert hat.
# Alle Boxplots müssen zwischen 0 und 1 liegen.

plot_normiert <- ggplot(df_belaege_normalized, aes(x = bewertungskategorie, y = score_normiert)) +
  geom_boxplot(fill = "grey90", width = 0.6, outlier.colour = "black", outlier.size = 1) +
  
  coord_flip() +
  
  # Fixe Skala 0-1
  scale_y_continuous(limits = c(0, 1), expand = c(0, 0.05)) +
  
  labs(
    title = NULL,
    subtitle = NULL,
    x = NULL, # Kriterien-Namen stehen an der Achse
    y = "Normierter Score (0 = schlecht, 1 = ideal)"
  ) +
  
  theme_classic(base_size = 11) +
  
  theme(
    # Gitterlinien: Vertikal wichtig (um 0.5 etc. zu sehen), Horizontal weg
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    panel.grid.major.y = element_blank(),
    
    # Achsen-Texte
    axis.text.y = element_text(color = "black", size = 10), # Kriterien-Namen
    axis.text.x = element_text(color = "black", size = 10)  # Zahlen 0-1
  )


#print(plot_normiert)

```


```{r}
#| include: false
ggsave("02_export/plots/check_02_normierte_scores_belaege.png", 
       plot = plot_normiert, width = 10, height = 7, dpi = 300)
```

![Normierte Scores der Belags-Kriterien (Min-Max-Skalierung)](02_export/plots/check_02_normierte_scores_belaege.png)
```{r}
#| include: false
# --- PRÜFUNG BLOCK 6 ---

print("--- Check: Liegen alle Scores zwischen 0 und 1? ---")
summary(df_belaege_normalized$score_normiert)
# Min sollte >= 0.00 sein, Max <= 1.00

print("--- Check: Sind 'initialkosten' im Boxplot-Datensatz enthalten? ---")
# Wir zählen, wie viele Datenpunkte wir pro Kategorie haben
df_belaege_normalized %>%
  filter(bewertungskategorie %in% c("initialkosten", "initialkosten_100a")) %>%
  count(bewertungskategorie)
```


## 7. MCA Berechnung

Ziel: Berechnung der besten geeigneten Beläge auf Basis der Kriterienbewertung (score_normiert * gewicht).

```{r}
# 7.1 Berechnung Score pro Kriterium
df_mca_calculated <- df_belaege_normalized %>%
  # Hier filtern wir alles raus, was kein Gewicht hat (z.B. reine Initialkosten)
  filter(!is.na(gewicht)) %>% 
  mutate(MCA_score_kriterium = score_normiert * gewicht)

# 7.2 Aggregation pro Szenario
# Wir summieren die Scores pro Belag und Szenario
df_mca_total <- df_mca_calculated %>%
  group_by(szenario, belagskategorie, option) %>%
  summarise(MCA_gesamtscore = sum(MCA_score_kriterium, na.rm = TRUE), .groups = 'drop') %>%
  mutate(MCA_gesamtscore = round(MCA_gesamtscore, 5)) %>%
  # Sortierung: Erst nach Szenario, dann nach Punktzahl
  arrange(szenario, desc(MCA_gesamtscore)) 

# Anzeigen der Ergebnisse
print("--- MCA Gesamtergebnis (Auszug) ---")
print(df_mca_total, n = 10)

# 7.3 - Auswahl der "Top 3 Sieger"
# Ziel: Für jedes der 4 Szenarien die Top 3 Beläge aus dem Gesamtranking
N_BELAEGE_AUSWAEHLEN <- 3

df_sieger_belaege <- df_mca_total %>%
  group_by(szenario) %>%
  # Wählt die N besten Beläge innerhalb jedes Szenarios aus
  slice_max(MCA_gesamtscore, n = N_BELAEGE_AUSWAEHLEN) %>%
  ungroup() %>%
  # Ordnen für bessere Lesbarkeit
  arrange(szenario, desc(MCA_gesamtscore))

# Anzeigen der 12 Sieger-Optionen (4 Szenarien * 3 Beläge)
print(paste("--- Top", N_BELAEGE_AUSWAEHLEN, "Beläge pro Szenario (für Zeitstrahl-Analyse) ---"))
print(df_sieger_belaege)

```


```{r}
#| include: false
# 7.4 Export der Tabellen
write_csv2(
  df_sieger_belaege,
  file = "02_export/tables/mca_top3_belaege_pro_szenario.csv"
)
write_csv2(
  df_mca_total, 
  file = "02_export/tables/mca_rangliste_alle_szenarien.csv"
)
write_csv2(
  df_mca_calculated, 
  file = "02_export/tables/mca_details_berechnung_alle_szenarien.csv"
)

# --- PRÜFUNG BLOCK 7 ---

print("--- Check: Enthält die Berechnung noch Zeilen ohne Gewicht? ---")
# Sollte 0 Zeilen ergeben
df_mca_calculated %>% filter(is.na(gewicht)) %>% count()

print("--- Check: Plausibilität Ergebnisse (S0 Baseline) ---")
# Zeig mir die Top 3 von Szenario 0
df_sieger_belaege %>% 
  filter(szenario == "Szenario 0: Baseline (IST)") %>%
  select(option, MCA_gesamtscore)

```

## 8. Darstellung

Ziel: Visualisierung der Ergebnisse - Datenaufbereitung für Plots 

```{r}

# --- Datenaufbereitung für Stacked Plot (einmalig & aufgehübscht) ---
df_mca_groups_belaege_plot <- df_mca_calculated %>%
  group_by(szenario, belagskategorie, option, group_id) %>%
  summarise(gruppen_score = sum(MCA_score_kriterium, na.rm = TRUE), .groups = 'drop') %>%
  left_join(df_mca_total, by = c("szenario", "belagskategorie", "option")) %>%
  mutate(group_label_clean = case_when(
    group_id == "langlebigkeit" ~ "Langlebigkeit",
    group_id == "wirtschaftlichkeit" ~ "Wirtschaftlichkeit",
    group_id == "multifunktionale_nutzungsqualitaet" ~ "Multifunktionale Nutzungsqualität",
    group_id == "umweltwirkung" ~ "Umweltwirkung",
    group_id == "kreislauffaehigkeit" ~ "Kreislauffähigkeit",
    TRUE ~ group_id
  ))

# 1. Farben definieren (Einheitlich zur gesamten Arbeit)
colors_kategorie <- c(
  "Asphalt" = "#084594", 
  "Pflaster" = "#4292c6", 
  "Sickerbeläge" = "#9ecae1")

# 2. Farben für Kriterien-Gruppen
colors_gruppen <- c(
  "Langlebigkeit" = "#08306b",                     # Dunkelstes Blau (Zeit)
  "Wirtschaftlichkeit" = "#2171b5",                # Dunkles Mittelblau (Geld)
  "Multifunktionale Nutzungsqualität" = "#4292c6", # Mittelblau
  "Umweltwirkung" = "#9ecae1",                     # Hellblau
  "Kreislauffähigkeit" = "#deebf7"                 # Sehr helles Blau/Grau
)
```


### 8.1 Plot: Szenario 0 - Baseline (IST)

```{r}
#| label: fig-mca-plots-belaege-s0
#| fig.width: 13
#| fig.height: 8
#| out.width: "95%"
#| dpi: 300

# --- Plot 1: Ranking (Szenario 0) ---
data_s0 <- df_mca_total %>% filter(szenario == "Szenario 0: Baseline (IST)")

plot_s0_rank <- ggplot(data_s0,
       aes(x = MCA_gesamtscore,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = belagskategorie)) + 
  geom_col(width = 0.7) +
  
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) + 
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_kategorie) +
  
  labs(
    title = NULL, 
    x = "MCA Score (0-1)", 
    y = NULL, 
    fill = "Kategorie"
  ) +

  theme_classic(base_size = 11) + 

  theme(
    legend.position = "bottom",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  )

# print(plot_s0_rank, width = 10, height = 7)
```


```{r}
#| include: false
ggsave("02_export/plots/mca_gesamtergebnis_s0_belaege.png",
       plot = plot_s0_rank, width = 10, height = 7, dpi = 300)
```

![Gesamtranking der Beläge im Szenario 0](02_export/plots/mca_gesamtergebnis_s0_belaege.png)

```{r}
# --- Plot 2: Stacked Detail (Szenario 0) ---
data_s0_groups <- df_mca_groups_belaege_plot %>% filter(szenario == "Szenario 0: Baseline (IST)")

plot_s0_stack <- ggplot(data_s0_groups,
       aes(x = gruppen_score,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = group_label_clean)) + 
  geom_col(position = "stack", width = 0.7) +
  
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_gruppen) +
  
  labs(
    title = NULL, 
    x = "Beitrag zum Score", 
    y = NULL, 
    fill = "Kriteriengruppe"
  ) +
  theme_classic(base_size = 11) + 
  theme(
    legend.position = "bottom",
    legend.text = element_text(size = 9),
    legend.box = "vertical",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  ) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE))

# print(plot_s0_stack)
```


```{r}
#| include: false
ggsave("02_export/plots/mca_detail_gruppen_s0_belaege.png",
       plot = plot_s0_stack, width = 10, height = 7, dpi = 300)
```

![Detailanalyse der Belags-Scores im Szenario 0](02_export/plots/mca_detail_gruppen_s0_belaege.png)


### 8.2 Plot: Szenario 1 - Forschungsfokus

```{r}
#| label: fig-mca-plots-belaege-s1
#| fig.width: 13
#| fig.height: 8
#| out.width: "95%"
#| dpi: 300

# --- Plot 1: Ranking (Szenario 1) ---
data_s1 <- df_mca_total %>% filter(szenario == "Szenario 1: Forschungsfokus")

plot_s1_rank <- ggplot(data_s1,
       aes(x = MCA_gesamtscore,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = belagskategorie)) + 
  geom_col(width = 0.7) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) + 
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_kategorie) +
  labs(
    title = NULL, 
    x = "MCA Score (0-1)", 
    y = NULL, 
    fill = "Kategorie"
  ) +
  theme_classic(base_size = 11) + 
  theme(
    legend.position = "bottom",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  )

# print(plot_s1_rank)
```


```{r}
#| include: false
ggsave("02_export/plots/mca_gesamtergebnis_s1_belaege.png",
       plot = plot_s1_rank, width = 10, height = 7, dpi = 300)
```
![Gesamtranking der Beläge im Szenario 1](02_export/plots/mca_gesamtergebnis_s1_belaege.png)


```{r}
# --- Plot 2: Stacked Detail (Szenario 1) ---
data_s1_groups <- df_mca_groups_belaege_plot %>% filter(szenario == "Szenario 1: Forschungsfokus")

plot_s1_stack <- ggplot(data_s1_groups,
       aes(x = gruppen_score,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = group_label_clean)) + 
  geom_col(position = "stack", width = 0.7) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_gruppen) +
  labs(
    title = NULL, 
    x = "Beitrag zum Score", 
    y = NULL, 
    fill = "Kriteriengruppe"
  ) +
  theme_classic(base_size = 11) + 
  theme(
    legend.position = "bottom",
    legend.text = element_text(size = 9), 
    legend.box = "vertical",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  ) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE))

# print(plot_s1_stack)
```


```{r}
#| include: false
ggsave("02_export/plots/mca_detail_gruppen_s1_belaege.png",
       plot = plot_s1_stack, width = 10, height = 7, dpi = 300)
```

![Detailanalyse der Belags-Scores im Szenario 1](02_export/plots/mca_detail_gruppen_s1_belaege.png)

### 8.3 Plot: Szenario 2 - Radikale Vermeidung

```{r}
#| label: fig-mca-plots-belaege-s2
#| fig.width: 13
#| fig.height: 8
#| out.width: "95%"
#| dpi: 300

# --- Plot 1: Ranking (Szenario 2) ---
data_s2 <- df_mca_total %>% filter(szenario == "Szenario 2: Radikale Vermeidung")

plot_s2_rank <- ggplot(data_s2,
       aes(x = MCA_gesamtscore,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = belagskategorie)) + 
  geom_col(width = 0.7) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) + 
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_kategorie) +
  labs(
    title = NULL, 
    x = "MCA Score (0-1)", 
    y = NULL, 
    fill = "Kategorie"
  ) +
  theme_classic(base_size = 11) + 
  theme(
    legend.position = "bottom",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  )

#print(plot_s2_rank)

```


```{r}
#| include: false
ggsave("02_export/plots/mca_gesamtergebnis_s2_belaege.png",
       plot = plot_s2_rank, width = 10, height = 7, dpi = 300)
```

![Gesamtranking der Beläge im Szenario 2](02_export/plots/mca_gesamtergebnis_s2_belaege.png)
```{r}
# --- Plot 2: Stacked Detail (Szenario 2) ---
data_s2_groups <- df_mca_groups_belaege_plot %>% filter(szenario == "Szenario 2: Radikale Vermeidung")

plot_s2_stack <- ggplot(data_s2_groups,
       aes(x = gruppen_score,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = group_label_clean)) + 
  geom_col(position = "stack", width = 0.7) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_gruppen) +
  labs(
    title = NULL, 
    x = "Beitrag zum Score", 
    y = NULL, 
    fill = "Kriteriengruppe"
  ) +
  theme_classic(base_size = 11) + 
  theme(
    legend.position = "bottom",
    legend.text = element_text(size = 9), 
    legend.box = "vertical",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  ) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE))

# print(plot_s2_stack)
```


```{r}
#| include: false
ggsave("02_export/plots/mca_detail_gruppen_s2_belaege.png",
       plot = plot_s2_stack, width = 10, height = 7, dpi = 300)

```

![Detailanalyse der Belags-Scores im Szenario 2](02_export/plots/mca_detail_gruppen_s2_belaege.png)

### 8.4 Plot: Szenario 3 - Radikaler Lebenszyklus

```{r}
#| label: fig-mca-plots-belaege-s3
#| fig.width: 13
#| fig.height: 8
#| out.width: "95%"
#| dpi: 300

# --- Plot 1: Ranking (Szenario 3) ---
data_s3 <- df_mca_total %>% filter(szenario == "Szenario 3: Radikaler Lebenszyklus")

plot_s3_rank <- ggplot(data_s3,
       aes(x = MCA_gesamtscore,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = belagskategorie)) + 
  geom_col(width = 0.7) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) + 
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_kategorie) +
  labs(
    title = NULL, 
    x = "MCA Score (0-1)", 
    y = NULL, 
    fill = "Kategorie"
  ) +
  theme_classic(base_size = 11) + 
  theme(
    legend.position = "bottom",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  )

# print(plot_s3_rank)
```


```{r}
#| include: false
ggsave("02_export/plots/mca_gesamtergebnis_s3_belaege.png",
       plot = plot_s3_rank, width = 10, height = 7, dpi = 300)
```
![Gesamtranking der Beläge im Szenario 3](02_export/plots/mca_gesamtergebnis_s3_belaege.png)

```{r}
# --- Plot 2: Stacked Detail (Szenario 3) ---
data_s3_groups <- df_mca_groups_belaege_plot %>% filter(szenario == "Szenario 3: Radikaler Lebenszyklus")

plot_s3_stack <- ggplot(data_s3_groups,
       aes(x = gruppen_score,
           y = fct_reorder(option, MCA_gesamtscore), 
           fill = group_label_clean)) + 
  geom_col(position = "stack", width = 0.7) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0.05)) +
  scale_y_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) +
  scale_fill_manual(values = colors_gruppen) +
  labs(
    title = NULL, 
    x = "Beitrag zum Score", 
    y = NULL, 
    fill = "Kriteriengruppe"
  ) +
  theme_classic(base_size = 11) + 
  theme(
    legend.position = "bottom",
    legend.text = element_text(size = 9), 
    legend.box = "vertical",
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color = "grey90", linewidth = 0.2),
    axis.text = element_text(color = "black"),
    plot.margin = margin(t = 5, r = 10, b = 5, l = 5, unit = "mm")
  ) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE))

# print(plot_s3_stack)
```


```{r}
#| include: false
ggsave("02_export/plots/mca_detail_gruppen_s3_belaege.png",
       plot = plot_s3_stack, width = 10, height = 7, dpi = 300)

```
![Detailanalyse der Belags-Scores im Szenario 3](02_export/plots/mca_detail_gruppen_s3_belaege.png)