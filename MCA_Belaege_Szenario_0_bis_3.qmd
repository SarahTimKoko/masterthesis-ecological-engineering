...
# -------------------------------------------------------
# PLOT: Lebenszyklus-Bilanz Beläge (FINAL FIX SKALEN)
# -------------------------------------------------------

# 1. Daten vorbereiten
df_plot_belaege_100a <- df_belaege_raw %>% 
  select(
    belagskategorie,       
    option,                
    kosten,                
  ...
  rename(
    belag = option,
    kategorie = belagskategorie 
  ) %>%
  
  pivot_longer(
    cols = c(kosten, kosten_100a, unterhalt_100a, co2_100a, graueEnergie_100a),
    names_to = "kriterium",
    values_to = "wert"
  ) %>%
  ...
  mutate(facet_label = case_when(
    kriterium == "kosten" ~ "1. Initialkosten [CHF/m²]",
    kriterium == "kosten_100a" ~ "2. Gesamtkosten 100a\n[CHF/m²]",
  ...
  mutate(facet_label = fct_relevel(facet_label, 
                                   "1. Initialkosten [CHF/m²]",
                                   "2. Gesamtkosten 100a\n[CHF/m²]",
                                   "3. Unterhaltskosten 100a\n[CHF/m²]",
                                   "4. CO2-Emissionen 100a\n[kg CO2-eq/m²]",
                                   "5. Graue Energie 100a\n[kWh/m²]")) %>%
  
  # Reihenfolge Kategorie
  mutate(kategorie = fct_relevel(kategorie, "Asphalt", "Pflaster", "Sickerbeläge")) %>%
  
  # Sortierung X-Achse
  arrange(kategorie, belag) %>%
  mutate(belag_sortiert = factor(belag, levels = unique(belag)))

# 2. Plot erstellen
plot_belaege_100a_final <- ggplot(df_plot_belaege_100a, 
       aes(x = belag_sortiert, 
           y = wert, 
           fill = kategorie)) + 
  
  geom_col(width = 0.7) +
  
  # --- FIX: facet_wrap mit 'scales = "free"' und 'ncol = 1' ---
  # 'ncol = 1': Alle 5 Grafiken untereinander (wie bei den Bäumen)
  # 'scales = "free"': JEDE Grafik hat ihre eigene Skala (x und y)
  facet_wrap(~facet_label, ncol = 1, scales = "free_y") +
  # ------------------------------------------------------------
  
  scale_fill_manual(values = c(
    "Asphalt" = "#084594",        # Dunkelblau
    "Pflaster" = "#4292c6",       # Mittelblau
    "Sickerbeläge" = "#9ecae1"    # Hellblau
  )) +
  
  labs(
    title = "Lebenszyklus-Bilanz: Ökonomie und Ökologie über 100 Jahre",
    subtitle = "Vergleich der kumulierten Werte (inkl. aller Sanierungszyklen).",
    x = "", 
    y = "Wert",
  ...
  theme(
    ...
    # Namen unten schräg stellen
    axis.text.x = element_text(angle = 45, hjust = 1, size = 9, color = "black"),
    panel.grid.major.x = element_blank(),
    
    # Rand anpassen (Links mehr Platz)
    plot.margin = unit(c(0.5, 0.5, 0.5, 1.5), "cm")
  )

# 3. Speichern (Hochformat für 5 Grafiken untereinander)
