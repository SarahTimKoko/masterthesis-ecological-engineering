...
#| label: fig-lebenszyklus-split
#| fig.width: 10
#| fig.height: 8
#| out.width: "100%"
#| warning: false

## 4. Explorative Datenanalyse (Mit Labels in den Balken)

# -------------------------------------------------------
# TEIL 1: KOSTEN (Gestapelt & Beschriftet)
# -------------------------------------------------------

# 1. Daten vorbereiten (Zyklen berechnen)
df_kosten_calc <- df_belaege %>%
  select(belagskategorie, option, initialkosten, initialkosten_100a, unterhalt_100a) %>%
  mutate(
    # Berechne Anzahl Zyklen
    n_zyklen = round(initialkosten_100a / initialkosten),
    total_kosten = initialkosten_100a + unterhalt_100a
  )

# 2. Datensatz "aufblasen"
# A) Investitions-Blöcke (Zyklen)
df_invest_expanded <- df_kosten_calc %>%
  uncount(n_zyklen) %>%
  mutate(
    kostenart = "Investitionskosten (pro Zyklus)",
    wert = initialkosten,
    # Label nur für Investition: Wert anzeigen
...
df_unterhalt <- df_kosten_calc %>%
  mutate(
    ...
    wert = unterhalt_100a,
    # Label für Unterhalt: Optional (hier leer lassen, um Fokus auf Zyklen zu legen)
    label_text = "" 
  )
...
plot_kosten_stacked <- ggplot(df_plot_kosten_final, 
  ...
  # HIER NEU: Beschriftung in den Balken
  geom_text(aes(label = label_text), 
            position = position_stack(vjust = 0.5), # Mittig im Block
            size = 2.5,                             # Kleine Schrift
            color = "white") +                      # Weisse Schrift auf dunklem Grund
  
  # Farben
  scale_fill_manual(values = c(
    "Investitionskosten (pro zyklus)" = "#084594", 
    "Unterhaltskosten (kumuliert 100a)" = "#9ecae1"
  )) +
  
  # HIER NEU: Feinere Skala (alle 100 CHF eine Linie)
  ...
  theme(
    legend.position = "bottom",
    # Horizontale Linien jetzt wichtiger denn je
    panel.grid.major.y = element_line(color = "grey90", linewidth = 0.2),
    panel.grid.major.x = element_blank(),
    axis.text.x = element_text(angle = 45, hjust = 1, size = 10, color = "black"),
...
# Wir brauchen 'patchwork' zum Zusammenfügen der Umwelt-Plots
if(!require(patchwork)) install.packages("patchwork")
library(patchwork)

# -------------------------------------------------------
# TEIL 1: KOSTEN (Gestapelt: Zyklen + Unterhalt)
# -------------------------------------------------------
...
# 2. Datensatz "aufblasen" für Zyklen-Darstellung
df_invest_expanded <- df_kosten_calc %>%
  uncount(n_zyklen) %>%
  mutate(
...
plot_kosten_stacked <- ggplot(df_plot_kosten_final, 
  ...
  # Balken mit weissen Trennlinien (macht die Zyklen sichtbar)
  geom_col(width = 0.7, color = "white", linewidth = 0.3) + 
  
  # Beschriftung: Mittig im Block (vjust = 0.5)
  geom_text(aes(label = label_text), 
  ...
  scale_y_continuous(breaks = seq(0, 3000, 200)) + # Skala angepasst
  scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 20)) +
