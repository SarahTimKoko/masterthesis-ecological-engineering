...
df_baeume_long <- df_baeume %>%
  pivot_longer(
    # Nimmt alle Spalten von lebensdauer bis recyclingpotential
...
# -------------------------------------------------------
# PLOT: Zielkonflikt Leistung vs. Kosten 
# -------------------------------------------------------

# 1. Daten vorbereiten (Wir nutzen df_baeume_long aus Block 3.1)
df_plot_tradeoff <- df_baeume_long %>%
  
  # Filtern der 4 relevanten Kriterien
  filter(bewertungskategorie %in% c("co2_bindung", "kuehlung", "regenwasserspeicherung", "groesse_baumrigolen")) %>%
  
  # Sicherheits-Schritt: Wert in Zahl umwandeln
  mutate(wert = as.numeric(wert)) %>%
  
  # Schöne Labels und Einheiten für die Grafik erstellen
  mutate(facet_label = case_when(
    ...
    bewertungskategorie == "regenwasserspeicherung" ~ "Leistung: Regenwasser [l/Tag]",
    bewertungskategorie == "groesse_baumrigolen" ~ "Kosten: Grösse Baumrigolen [m³]"
  )) %>%
  
  # Reihenfolge der 4 Grafiken erzwingen
  ...
  # Reihenfolge der Baum-Kategorien erzwingen
  mutate(baumkategorie = fct_relevel(baumkategorie, 
                                     "Grosskronige Arten", 
                                     "Mittelgrosse Arten", 
                                     "Kleine Arten")) %>%
  
  # Sortierung der X-Achse (Damit die Balken nach Kategorie gruppiert sind)
...
plot_tradeoff_4fach <- ggplot(df_plot_tradeoff, 
       aes(x = baumart_sortiert, 
           y = wert, 
           fill = baumkategorie)) +
  
  geom_col(width = 0.7) +
  
  # Faceting: 4 Grafiken untereinander
  # scales = "free_y" ist entscheidend für die unterschiedlichen Einheiten!
  ...
  labs(
    title = "Zielkonflikt: Ökologische Leistung vs. Raumbedarf",
    subtitle = "Vergleich der Baumarten: Hohe Leistung bedingt hohe Kosten (Raum).",
    x = "", 
    y = "Wert",
    fill = "Baumkategorie"
  ) +
  ...
  theme(
    ...
    plot.margin = unit(c(0.5, 0.5, 5.0, 3.0), "cm")
  )
...
ggsave("02_export/plots/check_03_zielkonflikt_baeume_4fach.png", 
       plot = plot_tradeoff_4fach, width = 10, height = 14, bg = "white")
