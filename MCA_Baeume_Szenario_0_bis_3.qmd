...
# -------------------------------------------------------
# PLOT MIT 4 KRITERIEN (CO2, Kühlung, Regenwasser, Kosten)
# -------------------------------------------------------

# 1. Daten vorbereiten
df_plot_tradeoff <- df_baeume_joined %>%
 filter(bewertungskategorie %in% c("co2_bindung", "kuehlung", "regenwasser", "groesse_baumrigolen")) %>%
  
  # Saubere Labels für die Facetten (Titel der 4 Grafiken)
  mutate(facet_label = case_when(
    bewertungskategorie == "co2_bindung" ~ "Leistung: CO2-Bindung [kg/a]",
  ...
  # Reihenfolge der Facetten erzwingen (Erst Leistung, dann Kosten ganz unten)
  mutate(facet_label = fct_relevel(facet_label, 
                                   "Leistung: CO2-Bindung [kg/a]",
                                   "Leistung: Kühlung [°C]",
                                   "Leistung: Regenwasser [l/Tag]",
                                   "Kosten: Grösse Baumrigolen [m³]")) %>%

  # Reihenfolge der Kategorien erzwingen (Gross -> Mittel -> Klein)
  mutate(baumkategorie = fct_relevel(baumkategorie, 
                                     "Grosskronige Arten", 
                                     "Mittelgrosse Arten", 
                                     "Kleine Arten")) %>%
  
  # Sortierung der X-Achse erzwingen
...
plot_tradeoff_4fach <- ggplot(df_plot_tradeoff, 
  ...
  # Faceting: 4 Grafiken UNTEREINANDER (ncol = 1)
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 45, hjust = 1, size = 7), # Namen schräg
    panel.grid.major.x = element_blank(),
    plot.margin = unit(c(0.5, 0.5, 1, 1), "cm")
  )


# 3. Speichern (Extra hoch, damit 4 Grafiken Platz haben)
